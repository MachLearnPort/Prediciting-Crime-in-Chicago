function [Out, header] = csv2struct(filename)
...
% Modified by Jeff Schwartzentruber

%% read xls file with a single header row
[~, ~, raw] = xlsread(filename);
nRow = size(raw,1);
nCol = size(raw,2);
header_O(1,:) = raw(1,:);   % Store header information
% Run loop to change the header names to Varnames
for i=1:size(raw,2)
    raw{1,i}=sprintf('X%d',i);
end
header = raw(1,:);
header_O(2,:)=raw(1,:); % add varible name to second row of header_0
...
for c=1:nCol
    ...
    if (ColNumeric(c))
        Out.(name) = num(:,iNum);
        %Create structured data struct
        X_struct.(name)=num(:,iNum);
        iNum = iNum+1;
    else
        Out.(name) = txt(:,iTxt);
        %Create unstructured data struct
        X_unstruct.(name)=txt(:,iTxt);
        iTxt = iTxt+1;
    end
end
header=header_O;
